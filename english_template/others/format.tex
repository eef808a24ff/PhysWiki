...
\newcommand{\Arctan}{\mathop{\mathrm{Arctan}}} % Arctan 函数
...
\newcommand{\bibentry}[1]{　\\ \vspace{-0.34cm}\input{./bibliographies/#1}}
% \newcommand{\ecaption}[1]{Figure: } % 英文图片标题
...
\newcommand{\laserdog}[2]{\section{#1}\label{#2}\input{./laserdog/#2}}
\newcommand{\Code}[1]{\subsubsection{#1.m}\lstinputlisting[language=MyMatlab]{./codes/#1}} % 插入代码（含标题）
\newcommand{\code}[1]{\lstinputlisting[language=MyMatlab]{./codes/#1}} % 插入代码（不含标题）
\newcommand{\pentry}[1]{\begin{mdframed}\textbf{Prerequisites\ } #1 \end{mdframed}\vspace{-0.3cm}\par}%预备知识的格式
\newcommand{\rentry}[1]{\begin{mdframed}\textbf{See also\ } #1 \end{mdframed}\vspace{-0.3cm}\par}%拓展阅读的格式
\newcommand{\eentry}[1]{\begin{mdframed}\textbf{Used in\ } #1 \end{mdframed}\vspace{-0.3cm}\par}%应用举例的格式
...
\newenvironment{Command}{\begin{mdframed}[backgroundcolor = background] \ttfamily\zihao{5}}{\end{mdframed}\vspace{-0.2cm}}
...
\newcommand{\Exaautorefname}{Example}
