...
\newcommand{\lambdabar}{\mkern-0.25mu {\bar{\phantom{a}}} \mkern -10.2mu\lambda} % 类似 \hbar
...
\newcommand{\bibli}{　\\ \vspace{-0.34cm}\input{bibliography}} % 文献词条
...
\newcommand{\pentry}[1]{\begin{mdframed}[linewidth=0.25mm]\textbf{预备知识\ } #1 \end{mdframed}\vspace{-0.3cm}\par}%预备知识的格式
\newenvironment{issues}[1]{\begin{mdframed}[linewidth=0.25mm]\begin{itemize}[leftmargin=0.8em]\setlength\itemsep{0em}\zihao{-5}#1}{\end{itemize}\end{mdframed}\vspace{-0.3cm}\par} % issue 框
\newcommand{\issueDraft}{\item 本词条处于草稿阶段．} % issue
\newcommand{\issueTODO}{\item 本词条存在未完成的内容．} % issue
\newcommand{\issueMissDepend}{本词条缺少预备知识， 初学者可能会遇到困难．} % issue
\newcommand{\issueAbstract}{本词条需要更多讲解， 便于帮助理解．}
\newcommand{\issueNeedCite}{\item 本词条需要更多参考文献．} % issue
\newcommand{\issueOther}[1]{\item #1} % issue

\newcommand{\needCite}{\textsuperscript{[需引用]}} % 需引用（正文）
\newcommand{\addTODO}[1]{\begin{mdframed}\zihao{-5}未完成：#1 \end{mdframed}\vspace{-0.3cm}\par} % 未完成（正文）
...
\makeatletter
\patchcmd{\tableofcontents}{\@starttoc{toc}}{\pdfbookmark[1]{目录}{anchor}\@starttoc{toc}}{}{}
\setlength{\@fptop}{5pt}
...
\renewcommand{\subsectionautorefname}{第}
\renewcommand{\thesubsection}{\arabic{subsection}节}
