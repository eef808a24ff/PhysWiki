...
\patchcmd{\thebibliography}{\chapter*}{\subsection}{}{} % 文献标题变为 subsection
...
\newcommand{\sinc}{\mathop{\mathrm{sinc}}} % sinc 函数
...
%\renewcommand\bibname{} % 修改参考文献列表的标题
...
\newcommand{\Code}[1]{\subsubsection{#1.m}\input{./codes/#1}} % 插入代码（含标题）
\newcommand{\code}[1]{\input{./codes/#1}} % 插入代码（不含标题）
\newcommand{\pentry}[1]{\begin{mdframed}\bb{预备知识\ } #1 \end{mdframed}\vspace{-0.3cm}}%预备知识的格式
\newcommand{\rentry}[1]{\begin{mdframed}\bb{拓展阅读\ } #1 \end{mdframed}\vspace{-0.3cm}}%拓展阅读的格式
\newcommand{\eentry}[1]{\begin{mdframed}\bb{应用举例\ } #1 \end{mdframed}\vspace{-0.3cm}}%应用举例的格式
